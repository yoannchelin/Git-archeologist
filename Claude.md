...
13. ~~**Entrypoints : schedulers + goroutine workers**~~ — **FAIT** : `emitFileEdges()` dans `parser.go` détecte `ast.GoStmt` → edge `spawns` (goroutines nommées) et appels `cron.*AddFunc/AddJob/Schedule` → edge `schedules` (callbacks cron). `find_entrypoints` expose `goroutine_workers` et `cron_jobs`. Les `go func() {...}()` anonymes ne produisent pas d'edge (non résolvables à un symbole nommé, comportement attendu).
